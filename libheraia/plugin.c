...
static heraia_plugin_t *get_plugin_handle(heraia_struct_t *main_struct, heraia_plugin_t *plugin,
    ...
    if (plugin != NULL && plugin->handle != NULL)
        {
            ...
            if (get_symbol == FALSE)
                {
                    log_message(main_struct, G_LOG_LEVEL_WARNING, "%s", g_module_error());
                    free_plugin(plugin);
                    return NULL;
                }
            else
...
static void load_one_plugin(heraia_struct_t *main_struct, const gchar *filename, guint plugins_nb)
{
    ...
    if ( (g_file_test(full_filename, G_FILE_TEST_IS_DIR) == FALSE) &&
         ...
         )
        {
            ...
            plugin = get_plugin_handle(main_struct, plugin, full_filename, filename);
            plugin = get_plugin_init_symbol(main_struct, plugin);

            init_plugin(main_struct, plugin, filename, plugins_nb);
        }
}


/**
 * @fn load_plugins(heraia_struct_t *main_struct)
...
void load_plugins(heraia_struct_t *main_struct)
{
    ...
    if (plugins_dir == NULL) /* error while openning the plugins directory */
        {
            log_message(main_struct, G_LOG_LEVEL_WARNING, "%s", error->message);
            g_error_free(error);
        }
    else
        {
...
/**
 * @fn refresh_all_plugins(heraia_struct_t *main_struct)
...
void refresh_all_plugins(heraia_struct_t *main_struct)
{
    ...
    GList *list = g_list_first(main_struct->plugins_list);
    heraia_plugin_t *plugin = NULL;
