...
static void connect_data_type_signals(heraia_struct_t *main_struct);
/* static data_type_t *retrieve_data_type_information(heraia_struct_t *main_struct); */
static void set_spinbutton_max_range(heraia_struct_t *main_struct);

static void close_data_type_window(heraia_struct_t *main_struct);
static void destroy_container_widget(treatment_container_t *tment_c, gboolean all);
static treatment_container_t *new_treatment_container(heraia_struct_t *main_struct);
static void create_treatment_container_widget(heraia_struct_t *main_struct, treatment_container_t *tment_c);
 ...
 *  main_struct needed to fill the combobox widget with available treatment list
 */
data_type_t *copy_data_type_struct(heraia_struct_t *main_struct, data_type_t *a_data_type)
{
    ...
    if (a_data_type != NULL)
        {
            ...
            while (list != NULL)
                {
                     ...
                     */
                    tment_c = new_treatment_container(main_struct);
            ...
            create_ud_data_interpretor_widgets(main_struct, data_type);
...
static void set_spinbutton_max_range(heraia_struct_t *main_struct)
{
    doc_t *doc = main_struct->current_doc;
    guint64 till_end = 0;
    ...
    if (gh != NULL)
        {
            size_spin_button = GTK_SPIN_BUTTON(heraia_get_widget(main_struct->xmls->main, "dt_size_spinbutton"));
 ...
 *  Needs main_struct structure to fill the combobox with available
 *  treatments
 */
static treatment_container_t *new_treatment_container(heraia_struct_t *main_struct)
{
    ...
    fill_treatment_combobox(tment_c->tment_list, main_struct->available_treatment_list);
    ...
    g_signal_connect(G_OBJECT(tment_c->plus), "clicked",
                     G_CALLBACK(add_treatment_container_widget), main_struct);
    g_signal_connect(G_OBJECT(tment_c->moins), "clicked",
                     G_CALLBACK(remove_treatment_container_widget), main_struct);
    g_signal_connect(G_OBJECT(tment_c->tment_list), "changed",
                     G_CALLBACK(cb_changed_in_treatment_container_widget), main_struct);
...
static void create_treatment_container_widget(heraia_struct_t *main_struct, treatment_container_t *tment_c)
{
    ...
    vbox = heraia_get_widget(main_struct->xmls->main, "dt_treatment_vbox");
...
static void add_treatment_container_widget(GtkWidget *widget, gpointer data)
{
    ...
    a_data_type = main_struct->current_data_type;

    /* Creates a new treatment container */
    new_container = new_treatment_container(main_struct);

    /* Create the last box that will contain one empty treatment container widget */
    create_treatment_container_widget(main_struct, new_container);
    ...
    refresh_hex_datas_entry(main_struct);
}
    ...
    if (list != NULL)
        {
            ...
            tment = find_treatment(main_struct->available_treatment_list, tment_name);
 ...
 */
void create_ud_data_interpretor_widgets(heraia_struct_t *main_struct, data_type_t *a_data_type)
{
    GtkWidget *vbox = NULL;
    ...
    vbox = heraia_get_widget(main_struct->xmls->main, "ud_type_vbox");
    gtk_box_pack_start(GTK_BOX(vbox), a_data_type->di_label, FALSE, FALSE, DT_BOX_PADDING);

    vbox = heraia_get_widget(main_struct->xmls->main, "ud_value_vbox");
 ...
 */
void show_data_type_window(heraia_struct_t *main_struct, data_type_t *a_data_type)
{
    treatment_container_t *new_container = NULL;
    GList *list = NULL;
    ...
    if (list == NULL) /* Creates only the first treatment_container */
        {
            log_message(main_struct, G_LOG_LEVEL_DEBUG, "Creating a new treatment container");
            ...
            while (list != NULL)
                {
                    ...
                    log_message(main_struct, G_LOG_LEVEL_DEBUG, "Creating a treatment container");
    ...
    fill_data_type_widgets(main_struct, a_data_type);
    set_spinbutton_max_range(main_struct);

    gtk_widget_show_all(heraia_get_widget(main_struct->xmls->main, "data_type_window"));
}
 ...
 */
void refresh_all_ud_data_interpretor(heraia_struct_t *main_struct, guint endianness)
{
    data_window_t *data_window = main_struct->current_DW;
    value_t *a_value = NULL;
    ...
    data_type_list = main_struct->data_type_list;
 ...
 *  structure.
 */
void refresh_hex_datas_entry(heraia_struct_t *main_struct)
{
    guchar *bin_datas = NULL;
    data_window_t *data_window = main_struct->current_DW;
    doc_t *doc = main_struct->current_doc;
    ...
    length = main_struct->current_data_type->size;
    ...
    if (doc->hex_widget != NULL)
        {
            ...
            if (result == TRUE)
                {
                    entry = heraia_get_widget(main_struct->xmls->main, "hex_datas_entry");
                    a_value = new_value_t(length, bin_datas);
                    values_list = g_list_append(values_list, (gpointer) a_value);
                    ...
                    tment_c_list = main_struct->current_data_type->treatment_c_list;
                    final = update_treatment_container_list_entries(tment_c_list, values_list);

                    /* data interpretor part */
                    if (final != NULL)
                        {
                            gtk_entry_set_text(GTK_ENTRY(main_struct->current_data_type->di_entry), (gchar *) final);
                        }
                }
 ...
 */
/*
void clear_data_type_widgets(heraia_struct_t *main_struct)
{
    GtkEntry *entry = NULL;
    GtkSpinButton *size_spin_button = NULL;
    gdouble min = 0.0;
    ...
    entry = GTK_ENTRY(heraia_get_widget(main_struct->xmls->main, "dt_name_entry"));
 ...
 */
void fill_data_type_widgets(heraia_struct_t *main_struct, data_type_t *a_data_type)
{
    GtkEntry *entry = NULL;
    GtkSpinButton *size_spin_button = NULL;
    gdouble min = 0.0;
...
static gboolean delete_data_type_window_event(GtkWidget *widget, GdkEvent  *event, gpointer data)
{
    ...
    close_data_type_window(main_struct);
...
static void close_data_type_window(heraia_struct_t *main_struct)
{
    ...
    /* Hiding the window */
    gtk_widget_hide(heraia_get_widget(main_struct->xmls->main, "data_type_window"));
}
...
static void dt_ok_button_clicked(GtkWidget *widget, gpointer data)
{
  ...
  if (g_ascii_strcasecmp(a_data_type->name, "") != 0)
      {
          data_type_list = is_data_type_name_already_used(main_struct->data_type_list, a_data_type->name);
          ...
          if (data_type_list == NULL)
              {
                   ...
                   */
                  log_message(main_struct, G_LOG_LEVEL_DEBUG, "Adding %s data type name to treeview", a_data_type->name);
                  add_data_type_name_to_treeview(main_struct, a_data_type->name);

                  /* data interpretor part */
                  gtk_label_set_text(GTK_LABEL(a_data_type->di_label), a_data_type->name);

                  main_struct->data_type_list = g_list_prepend(main_struct->data_type_list, a_data_type);
              }
          else
              {
                  /**
                   ...
                   */
                  log_message(main_struct, G_LOG_LEVEL_DEBUG, "Editing mode (%s exists)", a_data_type->name);

                  /* We kill the old widget's that are unsefull (we replace them) */
                  destroy_a_single_widget(main_struct->current_data_type->di_label);
                  destroy_a_single_widget(main_struct->current_data_type->di_entry);
                  ...
                  create_ud_data_interpretor_widgets(main_struct, a_data_type);
...
{
    heraia_struct_t *main_struct = (heraia_struct_t *) data;
    GtkSpinButton *dt_size_spinbutton = GTK_SPIN_BUTTON(heraia_get_widget(main_struct->xmls->main, "dt_size_spinbutton"));
    guint length = 0;
    ...
    if (main_struct != NULL)
        ...
        if (length >= DT_SPIN_MIN && length <= DT_SPIN_MAX)
            {
                if (main_struct->current_data_type != NULL)
                    {
                        main_struct->current_data_type->size = length;
                    }
            }
...
static void dt_name_entry_leave_notify_event(GtkWidget *widget, GdkEventCrossing *event, gpointer data)
{
    ...
    if (main_struct != NULL && main_struct->current_data_type != NULL)
        {
            entry = GTK_ENTRY(heraia_get_widget(main_struct->xmls->main, "dt_name_entry"));

            if (main_struct->current_data_type->name != NULL)
                {
                    g_free(main_struct->current_data_type->name);
                }

            main_struct->current_data_type->name = g_strdup(gtk_entry_get_text(entry));
  ...
  if (main_struct != NULL && main_struct->xmls != NULL && main_struct->xmls->main != NULL)
    {
      ...
      g_signal_connect(G_OBJECT(heraia_get_widget(main_struct->xmls->main, "data_type_window")), "delete_event",
               G_CALLBACK(delete_data_type_window_event), main_struct);

      g_signal_connect(G_OBJECT(heraia_get_widget(main_struct->xmls->main, "data_type_window")), "destroy",
               G_CALLBACK(destroy_data_type_window), main_struct);

      /* Ok button */
      g_signal_connect (G_OBJECT(heraia_get_widget(main_struct->xmls->main, "dt_ok_button")), "clicked",
            G_CALLBACK(dt_ok_button_clicked), main_struct);

      /* Cancel button */
      g_signal_connect (G_OBJECT(heraia_get_widget(main_struct->xmls->main, "dt_cancel_button")), "clicked",
            G_CALLBACK(dt_cancel_button_clicked), main_struct);

      /* When the size spinbutton changes its value */
      g_signal_connect(G_OBJECT(heraia_get_widget(main_struct->xmls->main, "dt_size_spinbutton")), "value-changed",
                       G_CALLBACK(dt_size_spinbutton_value_changed), main_struct);

      /* When the user leaves the name entry */
      g_signal_connect(G_OBJECT(heraia_get_widget(main_struct->xmls->main, "dt_name_entry")), "leave-notify-event",
                       G_CALLBACK(dt_name_entry_leave_notify_event), main_struct);
 ...
 */
void data_type_init_interface(heraia_struct_t *main_struct)
{
  if (main_struct != NULL)
    {
      connect_data_type_signals(main_struct);
    }
}
